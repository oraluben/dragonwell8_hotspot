...
class MethodFamily : public ResourceObj {
 ...
 public:
  ...
  bool contains_signature(Symbol* query) {
    for (int i = 0; i < _members.length(); ++i) {
      if (query == _members.at(i)._method->signature()) {
        return true;
      }
    }
    return false;
...
class FindMethodsByErasedSig : public HierarchyVisitor<FindMethodsByErasedSig> {
 ...
 public:
  ...
  void prepare(Symbol* name, Symbol* signature) {
    reset();
    _method_name = name;
...
static void generate_erased_defaults(
     ...
     InstanceKlass* klass, EmptyVtableSlot* slot, TRAPS) {
  ...
  visitor->prepare(slot->name(), slot->signature());
...
void DefaultMethods::generate_default_methods(
  ...
  for (int i = 0; i < empty_slots.length(); ++i) {
    ...
    generate_erased_defaults(&findMethodsByErasedSig, klass, slot, CHECK);
 }
#ifndef PRODUCT
  if (TraceDefaultMethods) {
